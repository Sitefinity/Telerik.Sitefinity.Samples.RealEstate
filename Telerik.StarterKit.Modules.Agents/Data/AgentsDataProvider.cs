...
{
    /// <summary>
    /// Base provider class for the agents module
    /// </summary>
    public abstract class AgentsDataProvider : ContentDataProviderBase, ILanguageDataProvider, IOpenAccessDataProvider
    {
        ...
        [MethodPermission(AgentsConstants.Security.PermissionSetName, AgentsConstants.Security.Create)]
        public abstract AgentItem CreateAgent();
        /// <summary>
        ...
        /// <returns>Newly created agent item in transaction</returns>
        [MethodPermission(AgentsConstants.Security.PermissionSetName, AgentsConstants.Security.Create)]
        public abstract AgentItem CreateAgent(Guid id);
        /// <summary>
        ...
        [ValuePermission(AgentsConstants.Security.PermissionSetName, AgentsConstants.Security.View)]
        public abstract AgentItem GetAgent(Guid id);
        /// <summary>
        /// Get a query of all agents in this provider
        /// </summary>
        /// <returns>Query of all agents</returns>
        [EnumeratorPermission(AgentsConstants.Security.PermissionSetName, AgentsConstants.Security.View)]
        public abstract IQueryable<AgentItem> GetAgents();
        /// <summary>
        /// Delete an agent
        /// </summary>
        /// <param name="agent">Agent to delete</param>
        [ParameterPermission("agent", AgentsConstants.Security.PermissionSetName, AgentsConstants.Security.Delete)]
        public abstract void DeleteAgent(AgentItem agent);
        ...
        public override void DeleteItem(object item)
        {
            ...
            if (itemType == typeof(AgentItem))
            {
                this.DeleteAgent((AgentItem)item);
                return;
            }
        ...
        public override IEnumerable GetItemsByTaxon(Guid taxonId, bool isSingleTaxon, string propertyName, Type itemType, string filterExpression, string orderExpression, int skip, int take, ref int? totalCount)
        {
            if (itemType == typeof(AgentItem))
            {
                ...
                IQueryable<AgentItem> query =
                    (IQueryable<AgentItem>)this.GetItems(itemType, filterExpression, orderExpression, 0, 0, ref internalTotCount);
                if (isSingleTaxon)
                {
        ...
        {
            if (itemType == typeof(AgentItem))
                return typeof(AgentItemUrlData);
            throw GetInvalidItemTypeException(itemType, this.GetKnownTypes());
        }
